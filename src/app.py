...
class FlockLedgerApp:
    ...
    def open_file(self):
        ...
        if file_path:
            try:
                # Leer CSV especificando tipos de datos para evitar conversiones automáticas
                self.df = pd.read_csv(file_path, dtype=str, na_filter=False)
                self.current_file = file_path
                
                # Crear explotación desde el CSV
                # Usar el nombre del archivo como código de explotación
    ...
    def save_file(self):
        ...
        if self.explotacion_actual is None:
        ...
        if self.current_file is None:
            ...
            try:
                # Convertir explotación a DataFrame
    ...
    def display_data(self):
        ...
        # Convertir explotación a DataFrame para obtener columnas
        df = self.explotacion_actual.to_dataframe()
    ...
    def add_row(self):
        ...
        numero_orden = max([o.numero_orden for o in self.explotacion_actual.ovejas], default=0) + 1
        nueva_oveja = Oveja(
    ...
    def delete_row(self):
        ...
        if messagebox.askyesno("Confirmar", "¿Desea eliminar la fila seleccionada?"):
            for item in selected:
                # Obtener el número de orden de la oveja
    ...
    def search_data(self):
        ...
        if not search_term or self.explotacion_actual is None:
            self.display_data()
            return
        
        # Convertir a DataFrame para buscar
        df = self.explotacion_actual.to_dataframe()
